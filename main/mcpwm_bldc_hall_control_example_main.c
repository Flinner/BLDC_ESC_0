...
// #define BLDC_MCPWM_PERIOD              1000      // THIS IS TEH BEST FREQ ?
//  500 works too.
// #define BLDC_MCPWM_TIMER_RESOLUTION_HZ 10000000 // 10MHz, 1 tick = 0.1us
#define BLDC_MCPWM_TIMER_RESOLUTION_HZ 1000000 // 10MHz, 1 tick = 0.1us
#define BLDC_MCPWM_PERIOD 1000                 // 50us, 20KHz
#define BLDC_SPIN_DIRECTION_CCW false          // define the spin direction
#define BLDC_SPEED_UPDATE_PERIOD_US 200000     // 200ms

#define BLDC_DRV_EN_GPIO 13
#define BLDC_DRV_FAULT_GPIO 4

// A
#define BLDC_PWM_UH_GPIO 23 // 47
#define BLDC_PWM_UL_GPIO 22 // 21
// B
#define BLDC_PWM_VH_GPIO 21 // 14
#define BLDC_PWM_VL_GPIO 19 // 13
// C
#define BLDC_PWM_WH_GPIO 17 // 12
#define BLDC_PWM_WL_GPIO 16 // 11

#define HALL_CAP_U_GPIO 25 // 4
#define HALL_CAP_V_GPIO 14 // 5
#define HALL_CAP_W_GPIO 12 // 6
...
#define HALL_POWER 32
#define SPEED_INPUT 36
...
static inline uint32_t bldc_get_hall_sensor_value(bool ccw) {
  uint32_t hall_val = gpio_get_level(HALL_CAP_U_GPIO) * 4 +
                      gpio_get_level(HALL_CAP_V_GPIO) * 2 +
                      gpio_get_level(HALL_CAP_W_GPIO) * 1;
  return ccw ? hall_val ^ (0x07) : hall_val;
}

static bool IRAM_ATTR bldc_hall_updated(mcpwm_cap_channel_handle_t cap_channel,
...
static void bldc_set_phase_up_vm(mcpwm_gen_handle_t (*gens)[2]) {
  ...
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_U][BLDC_MCPWM_GEN_INDEX_HIGH], -1, true);
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_U][BLDC_MCPWM_GEN_INDEX_LOW], -1, true);
  // V+ = 0, V- = 1
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_V][BLDC_MCPWM_GEN_INDEX_HIGH], 0, true);
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_V][BLDC_MCPWM_GEN_INDEX_LOW], 1, true);
  // W+ = 0, W- = 0
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_W][BLDC_MCPWM_GEN_INDEX_HIGH], 0, true);
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_W][BLDC_MCPWM_GEN_INDEX_LOW], 0, true);
}
...
static void bldc_set_phase_wp_um(mcpwm_gen_handle_t (*gens)[2]) {
  // U+ = 0, U- = 1
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_U][BLDC_MCPWM_GEN_INDEX_HIGH], 0, true);
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_U][BLDC_MCPWM_GEN_INDEX_LOW], 1, true);
  ...
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_V][BLDC_MCPWM_GEN_INDEX_HIGH], 0, true);
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_V][BLDC_MCPWM_GEN_INDEX_LOW], 0, true);

  // W+ = PWM, W- = _PWM_
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_W][BLDC_MCPWM_GEN_INDEX_HIGH], -1, true);
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_W][BLDC_MCPWM_GEN_INDEX_LOW], -1, true);
}
...
static void bldc_set_phase_wp_vm(mcpwm_gen_handle_t (*gens)[2]) {
  ...
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_U][BLDC_MCPWM_GEN_INDEX_HIGH], 0, true);
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_U][BLDC_MCPWM_GEN_INDEX_LOW], 0, true);
...
static void bldc_set_phase_vp_um(mcpwm_gen_handle_t (*gens)[2]) {
  ...
  // V+ = PWM, V- = _PWM_
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_V][BLDC_MCPWM_GEN_INDEX_HIGH], -1, true);
  mcpwm_generator_set_force_level(
      gens[BLDC_MCPWM_OP_INDEX_V][BLDC_MCPWM_GEN_INDEX_LOW], -1, true);
...
static const bldc_hall_phase_action_t s_hall_actions[] = {
    [2] = bldc_set_phase_up_vm, [6] = bldc_set_phase_wp_vm,
    [4] = bldc_set_phase_wp_um, [5] = bldc_set_phase_vp_um,
    [1] = bldc_set_phase_vp_wm, [3] = bldc_set_phase_up_wm,
};

static void update_motor_speed_callback(void *arg) {
  // static int step = 5;
  // static int cur_speed = 40;
  // if ((cur_speed + step) > 300 || (cur_speed + step) < 0) {
  // step *= -1;
  //}
  // cur_speed += step;

  uint32_t speed = adc1_get_raw(ADC_CHANNEL_0);
  speed = speed > 300 ? 300 : speed;
  ESP_LOGI(TAG, "SPEED IS: %" PRIu32, speed);

  mcpwm_cmpr_handle_t *cmprs = (mcpwm_cmpr_handle_t *)arg;
  for (int i = 0; i < 3; i++) {
    ESP_ERROR_CHECK(mcpwm_comparator_set_compare_value(cmprs[i], speed));
  }
}

void app_main(void) {
  gpio_set_direction(HALL_POWER, GPIO_MODE_OUTPUT);
  gpio_set_level(HALL_POWER, 1);

  adc1_config_width(ADC_WIDTH_BIT_9);
  adc1_config_channel_atten(ADC1_CHANNEL_0, ADC_ATTEN_DB_6);
  // gpio_set_direction(SPEED_INPUT, GPIO_MODE_INPUT);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    // set compare value to 0, we will adjust the speed in a period timer
    // callback
  ...
  mcpwm_gpio_fault_config_t gpio_fault_config = {
      ...
      .group_id = 0,
      .flags.active_level =
          0, // low level means fault, refer to DRV8302 datasheet
  ...
  for (int i = 0; i < 3; i++) {
    ESP_ERROR_CHECK(
        mcpwm_operator_set_brake_on_fault(operators[i], &brake_config));
  }
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 2; j++) {
      ...
      ESP_ERROR_CHECK(
          mcpwm_new_generator(operators[i], &gen_config, &generators[i][j]));
    }
  }
  ...
  mcpwm_dead_time_config_t dt_config = {
      .posedge_delay_ticks = 5 // 5,
  };
  for (int i = 0; i < 3; i++) {
    ESP_ERROR_CHECK(mcpwm_generator_set_dead_time(
        generators[i][BLDC_MCPWM_GEN_INDEX_HIGH],
        generators[i][BLDC_MCPWM_GEN_INDEX_HIGH], &dt_config));
  }
  ...
  for (int i = 0; i < 3; i++) {
    ESP_ERROR_CHECK(mcpwm_generator_set_dead_time(
        generators[i][BLDC_MCPWM_GEN_INDEX_HIGH],
        generators[i][BLDC_MCPWM_GEN_INDEX_LOW], &dt_config));
  }
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 2; j++) {
      ESP_ERROR_CHECK(
          mcpwm_generator_set_force_level(generators[i][j], 0, true));
    }
  }
  ...
  const int cap_chan_gpios[3] = {HALL_CAP_U_GPIO, HALL_CAP_V_GPIO,
                                 HALL_CAP_W_GPIO};
  for (int i = 0; i < 3; i++) {
    cap_channel_config.gpio_num = cap_chan_gpios[i];
    ESP_ERROR_CHECK(mcpwm_new_capture_channel(cap_timer, &cap_channel_config,
                                              &cap_channels[i]));
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    ESP_ERROR_CHECK(mcpwm_capture_channel_register_event_callbacks(
        cap_channels[i], &cbs, task_to_notify));
  }
  ...
  ESP_ERROR_CHECK(
      esp_timer_start_periodic(periodic_timer, BLDC_SPEED_UPDATE_PERIOD_US));
  ...
  while (1) {
    ...
    if (hall_sensor_value >= 1 && hall_sensor_value <= 6) {
      ...
      ESP_LOGE(TAG, "invalid bldc phase, wrong hall sensor value:%" PRIu32,
               hall_sensor_value);
    }
